...
public class NetworkServer : MonoBehaviour
{
	...
	private Netwk_socket up_socket = new Netwk_socket("192.168.1.66", "3000");
	private Netwk_socket down_socket = new Netwk_socket("192.168.1.66", "4000");
	...
	//float quaternionslerppercentage = (float)0.4;
	//float shift_inter_percent = (float)0.5;
	...
	void Update()
	{
		//Vector3 add_pos = new Vector3(0, 0, (float)0.01);
		//yyy = yyy + add_pos;
		//ObjectPositions[0] = yyy;
		//ObjectHasMoved[0] = true;
		...
		//TrackingList.SetObjsTR(ObjectTargetPosition, ObjectTargetRotation, ObjectHasMoved);
		//for (int i = 0; i < ObjectHasMoved.Count; i++)
		//{
		//	ObjectHasMoved[i] = false;
		//}
	...
	public async void Run_message()
	{
		...
		while (true)
		{
			//print("in while true 1");
			string mess = this.up_socket.Recv();
			//print("in while true 2");
			//print(mess);
			if (mess.Length == 1024)
			{
		...
		var task = Task.Run(() =>
		{
			...
			if (mess.Length == 1024)
			{
				...
				if (my_decode.control_command == Control_enum.create_axis)
				{
					...
					print(add_pos);
					//print("trying to set axis");
					this.ad_po = add_pos;
					this.has_ad_po = false;

					//print("successfully set axis");
